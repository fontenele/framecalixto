...
class json {
	...
	*/
	function codificarArray(array $var){
		...
		return sprintf('[%s]', join(',', array_map(array($this, 'codificar'), $var)));
	}
	/**
	...
	*/
	function montaChave($name, $value){
		return (sprintf("%s:%s", $this->codificar(strval($name)), $this->codificar($value)));
	}        
}
?>
